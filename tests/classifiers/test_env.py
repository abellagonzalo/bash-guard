#!/usr/bin/env python3
"""Unit tests for guard/classifiers/env.py.
...
what the orchestrator passes in. Stdlib-only, like the sibling suites.
    ...
    python3 tests/classifiers/test_env.py     # -> prints a summary, exits 1 on any failure
"""
...
from guard.classifiers import env  # noqa: E402

# (label, tokens, expected_ok) -- only prints or assigns; running a command defers.
CASES = [
    ("env bare", ["env"], True),
    ("env assign", ["env", "FOO=1"], True),
    ("env -0", ["env", "-0"], True),
    ("env run", ["env", "FOO=1", "bash"], False),
    ("env opt", ["env", "-i"], False),
]
...
def main() -> int:
    ...
    for label, tokens, expected in CASES:
        ok, _reason = env.classify(tokens)
