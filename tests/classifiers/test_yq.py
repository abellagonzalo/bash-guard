#!/usr/bin/env python3
"""Unit tests for guard/classifiers/yq.py.
...
what the orchestrator passes in. Stdlib-only, like the sibling suites.
    ...
    python3 tests/classifiers/test_yq.py     # -> prints a summary, exits 1 on any failure
"""
...
from guard.classifiers import yq  # noqa: E402

# (label, tokens, expected_ok) -- yq edits in place via -i/--inplace;
# -o is an output FORMAT (harmless).
CASES = [
    ("yq read", ["yq", ".a", "f"], True),
    ("yq output fmt", ["yq", "-o=json", ".a", "f"], True),
    ("yq -i", ["yq", "-i", ".a=1", "f"], False),
    ("yq --inplace", ["yq", "--inplace", ".a=1", "f"], False),
    ("yq bundled i", ["yq", "-iP", ".a=1", "f"], False),
]
